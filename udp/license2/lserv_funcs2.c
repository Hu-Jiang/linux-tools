...
/*
 * do_goodbye take back ticket client is returning
 * in msg_p, message received from client
 * results: ptr to response
...
char* do_goodbye(char* msg_p) {
    ...
    // the ticket out of the message, which looks like: GBYE pid.slot
...
/* 
 * do_validate validate client's ticket
 * in msg, message received from client
 * results: ptr to response
 */
