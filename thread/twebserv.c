/* webserv.c -- a threaded minimal web server(version 0.2)
 ...
 * building: cc twebserv.c socklib.c -lpthread -o tws
 */
...
int main(int ac, char* av[]) {
    ...
    if (ac == 1) {
        fprintf(stderr, "usuag: ws portnum\n");
        exit(1);
    }
...
// initialize the status variables and set the thread attribute to detached
int setup(pthread_attr_t* attrp) {
...
// skip_rest_of hander skip over all request info until a CRNL is seen
int skip_rest_of_hander(FILE* fp) {
    char buf[BUFSIZ];
    while (fgets(buf, BUFSIZ, fp) != NULL && strcmp(buf, "\r\n") != 0);
}

/*
 * process_rq do what the request asks for and write reply to fd
 ...
 * not_implemented unimplemented HTTP command
 ...
 * file_type returns the 'extension': cat uses it
 */
